...
export interface IOrderListTableProps {
    ...
    columns: {title: string, column: Exclude<keyof Order, "orders" | "tableInfo">}[]; // 这里 我把column的类型 修改了下因为 Order里面有几个属性类型定义的是对象是复发再render的是后作为内容渲染的所以我类型里面提出了后续你需要处理下比如tableInfo 和orders
    orders: Order[];
    paging: ReturnType<typeof Paging>;
    actions?: string[];
    onActionClick?: (action: string, order: Order) => void;
    renderItem?:(colKey:string,value:any)=>string | JSX.Element
}
...
export const OrderListTable: React.FC<IOrderListTableProps> = ({
    ...
    return (<>
                            ...
                            <tr key={index} className='z-40 relative'>
                                {
                                    columns.map((column, index) => {
                                        ...
                                        if(renderItem){
                                            value = renderItem(colKey,value)
                                        }
                                        return <td key={column.column}>{value}</td>
                                    })
                                }
                                {
                                    actions.length > 0 
        ...
        <div className="sticky bottom-0 px-6 h-12 border-t border-black border-opacity-10 border-solid mt-3 z-[999] flex justify-end items-center"
            ...
            ref={pagingRef}
        >
            {/* <Paging current={currentPage} total={totalPage} onChange={(page) => onPagingChange(page)} /> */}
            {paging}
        </div>
    </>)
}
